...
def find_session_manager_plugin() -> Optional[str]:
    """Resolve Session Manager Plugin reliably from desktop GUI apps on Windows/macOS."""
    import shutil

    override = (os.environ.get("SESSION_MANAGER_PLUGIN_PATH") or "").strip().strip('"')
    if override and os.path.isfile(override):
        return override
    ...
    if os.name == "nt":
        ...
        candidates.extend(["/usr/local/bin/session-manager-plugin", "/usr/bin/session-manager-plugin"])
...
def atlas_subprocess_env() -> Dict[str, str]:
    """Return an environment with CLI/plugin paths available to GUI-launched processes."""
    env = os.environ.copy()
    path_parts = [p for p in (env.get("PATH") or "").split(os.pathsep) if p]
    extra = []
    if sys.platform == "darwin":
        extra = ["/opt/homebrew/bin", "/usr/local/bin", "/usr/local/sessionmanagerplugin/bin", "/usr/bin", "/bin", "/usr/sbin", "/sbin"]
    elif os.name == "nt":
        extra = [r"C:\Program Files\Amazon\AWSCLIV2", r"C:\Program Files\Amazon\SessionManagerPlugin\bin"]
    for item in extra:
...
def run_cmd(cmd: List[str], env: Optional[Dict[str, str]] = None, timeout: int = 1800) -> Tuple[int, str, str]:
    try:
        ...
        p = subprocess.Popen(
            ...
            text=True,
            env=env or atlas_subprocess_env(),
            shell=False,
...
def api_diagnostics():
    ...
    try:
        plugin_exe = find_session_manager_plugin()
        if not plugin_exe:
            raise FileNotFoundError("session-manager-plugin not found")
        code, out, err = run_cmd([plugin_exe, "--version"], timeout=10)
        plugin_ok = code == 0
...
def cluster_members():
    ...
    if (request.args.get("format") or "").strip().lower() == "json":
        ...
        for m in members:
            ...
            display_endpoint = str(route53_records[0]) if route53_records else endpoint_raw
            ...
            payload_members.append({
                ...
                "remote_port": remote_port,
                "local_port": local_port,
                "aws_target": str(m.get("aws_target") or m.get("name") or ""),
                "can_forward": can_forward,
            })
        return jsonify({
            "ok": True, "title": title, "profile": profile, "region": region, "env": env,
            "target_type": target_type, "cluster_id": cluster_id,
            "cluster_endpoint": str(found.get("endpoint") or ""),
            "cluster_port": int(found.get("remote_port") or 0),
...
def start_ssm_shell(profile: str, region: str, instance_id: str) -> int:
    ...
    p = subprocess.Popen(cmd, shell=False, cwd=BASE_DIR, env=atlas_subprocess_env())
    return int(p.pid or 0)
