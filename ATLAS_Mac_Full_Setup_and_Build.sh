...
echo "[5/10] Validating launcher_desktop.py macOS runtime handling..."

if ! grep -q 'sys.platform == "darwin"' launcher_desktop.py; then
  echo "ERROR: macOS platform handling is missing from launcher_desktop.py"
  exit 1
fi

if ! grep -q '/usr/local/sessionmanagerplugin/bin' launcher_desktop.py; then
  echo "ERROR: macOS Session Manager PATH handling is missing from launcher_desktop.py"
  exit 1
fi

if grep -q 'webview.start(gui="edgechromium")' launcher_desktop.py && ! grep -q 'elif os.name == "nt"' launcher_desktop.py; then
  echo "ERROR: Edge Chromium appears to be forced globally. macOS must use native pywebview."
  exit 1
fi

echo "launcher_desktop.py macOS validation OK."
